...
In this report we will be analyzing data from the 311 Calls, restaurant violations, housing violaitons and the population by zipcode. The information is provided for New York city and for the year 2016. 

All the files are allocated in the Stern big data cluster in the Hive MSBA databease. The files have the following names:
...
Throughout the document, the reader will find **in bold** dificulties we encounter during the process.
...
This initial step in this process is to understand the data. The data is divided into four separate data tables. Each data table must to be reviewed for data clarity and consistency. Below is the step-by-step approach for data understanding:
  ...
  1. For every table we have built a data dictionary where we describe the follwing for each attribute.

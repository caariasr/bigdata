...
allbyzipmonth <- readRDS("allbyzipmonth.rds")
...
as 311 restaurant calls, we can conclude that there is not a strong linear relationship
between restaurant 311 calls and violations. Nevertheless we can test the null hypothesis
that the population's true linear correlation is zero:
...
**Fast conclusions**: October was a different behavior. November and December have no data
Maybe looking by month is not a good idea since the inspections dates are lagged 
compared to the 311 call dates (Consider bringing another grouping variable). Finally,
it looks like they don't do inspections at the end of the year (could also be a reason
for the different behavior in October, which is less calls and less violations, 
the calls part shouldn't happen though).
...
**Fast conclusions:** There are differences by month, October again has lower calls and violations than the rest. Low populated zip codes tend to have
close to 0 calls and violations
...
**Fast conclusions:** There are differences by month, October again has lower calls and violations than the rest. There seems to be a relationship between calls and population in terms of the range of the calls, the maximum value observed in calls increases as the population increaser, around 
60000 it starts decreasing again (And we see an outlier of 200 calles in a zipcode close to 90000 people). This effect is not seen with the violations.
